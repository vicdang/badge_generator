...
FACE_CASCADE_PATH = "Haar Cascade/haarcascade_frontalface_default.xml"
ORIENTATION_TAG = 274
TEXT_RENDER_CONFIGS = {
...
class ImageMaker:
    ...
    def __init__(self, name: str, arg: argparse.Namespace, conf) -> None:
        ...
        self.tmp_path = CUR_PATH / conf.get("general", "tmppath")
    ...
    def execute(self) -> None:
        ...
        try:
            ...
            self.bg_img = Image.new(
                'RGBA',
                (tpl_w, tpl_h),
                self.conf.get("general", "backgroundcolor")
            )
...
def main(args: argparse.Namespace, config) -> None:
    ...
    tmp_path = CUR_PATH / config.get("general", "tmppath")
    cv_path = CUR_PATH / config.get("general", "convertedpath")
    format_list = uT.get_list_file_extensions()
    ...
    if hasattr(args, 'enable_crawler') and args.enable_crawler:
        ...
        download_stats = img_manager.download_missing_images(files, use_threading=True)
